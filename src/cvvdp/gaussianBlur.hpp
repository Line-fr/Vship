...
//GAUSSIANSIZE is 8 which means 0.1% error for sigma 3 (exp(-64/9))
...
__device__ void GaussianSmart_Device(float* tampon, int64_t x, int64_t y, int64_t width, int64_t height, float* gaussiankernel, float* gaussiankernel_integral){
    const int thx = threadIdx.x;
    const int thy = threadIdx.y;
