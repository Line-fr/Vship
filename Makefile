...
buildFFVSHIP: src/FFVship.cpp .FORCE
	clang++ src/FFVship.cpp -g -std=c++17 $(ffvshipincludeheader) -L ./ $(ffvshiplibheader) -o FFVship$(exeend)

build: src/VshipLib.cpp .FORCE
	hipcc src/VshipLib.cpp -g -std=c++17 -I "$(current_dir)include" --offload-arch=native -shared $(fpicamd) -o "$(current_dir)libvship$(dllend)"
...
buildall: src/VshipLib.cpp .FORCE
	hipcc src/VshipLib.cpp -g -std=c++17 -I "$(current_dir)include" $(fatbincompressamd) --offload-arch=$(HIPARCH) -shared $(fpicamd) -o "$(current_dir)libvship$(dllend)"
